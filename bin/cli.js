...
let defaultOptions = {
  ...
  'domains': {
    demandOption: true,
    describe:
      'Domain names to apply. For multiple domains use space separated list of domains as a parameter.',
    type: 'array'
  },
  'domain-key-path': {
  ...
  'rsa-key-size': {
    coerce: (arg) => {
      if (arg < 2048 || !isFinite(arg)) {
        throw new Error(
          'Invalid RSA key size option. Must be 2048 or greater.'
        );
      }
  ...
  'server': {
    ...
    describe:
      'ACME Directory Resource URI. Default: staging server. ' +
      'Use "production" to connect to the production server.',
    choices: [
      greenlock.stagingServerUrl,
      greenlock.productionServerUrl,
      'staging',
      'production'
    ],
    type: 'string'
...
let argv = yargs
  .usage('$0 <cmd> [args]')
  .command('certonly', 'Issue/renew certificate(s)', defaultOptions)
  .command('renew', 'Renew certificate(s)', defaultOptions)
  .demandCommand(
    1,
    1,
...
mkdirp(options.configDir, (error) => {
  if (error) {
    console.error(
      'Could not create --config-dir "' + options.configDir + '"',
      error.code
    );
  } else {
    main();
...
async function generateCertificate() {
  try {
    const certs = await LetsEncrypt.generateCertificate(options);
...
async function renewCertificate() {
  try {
    const certs = await LetsEncrypt.renewCertificate(options);
    printCertificateInfo(certs);
  } catch (error) {
    console.error(error);
  }
...
/**
 * Shows information about the generated certificates in the console.
 * @param {!object} certs The certificates for which information should be
 *   printed
 */
function printCertificateInfo(certs) {
